...
LIGHT_CAL_PATH = os.path.join(SAVE_PATH, 'light_cal.json')
...
class EvolverNamespace(BaseNamespace):
    ...
    def on_broadcast(self, data):
        ...
        with open(LIGHT_CAL_PATH) as f:
            light_cal = json.load(f)
    ...
    def on_activecalibrations(self, data):
        ...
        for calibration in data:
            ...
            elif calibration['calibrationType'] == 'light':
                file_path = LIGHT_CAL_PATH
            else:
                continue
    ...
    def transform_data(self, data, vials, od_cal, temp_cal):
        ...
        # turning data into floats from strings
        # we take OD before this
        ...
        for x in vials:
            ...
            try:
                if od_cal['type'] == SIGMOID:
                    ...
                    # print('Vial '+str(x)+':: ',od_data[x],od_coefficients[0], float(od_data[x]) - od_coefficients[0])
                    if x == 3:
                        print( "\nVial "+str(x)+"\nraw OD::",od_data[x],"\nOD Coefficients:: ",od_coefficients)
                        print(od_data[x],'-',od_coefficients[0],'=', (float(od_data[x]) - od_coefficients[0]))
    ...
    def initialize_exp(self, vials, always_yes=False):
        ...
        if exp_continue == 'n':
            ...
            os.makedirs(os.path.join(EXP_DIR, 'light'))
            ...
            for x in vials:
                ...
                # make light data file
                self._create_file(x, 'light')
                # make light configuration file
                self._create_file(x, 'light_config',
                                  defaults=[exp_str,
                                            "0,{0}".format(LIGHT_INITIAL[x])])
                # make pump log file
    ...
    def check_for_calibrations(self):
        result = True
        if not os.path.exists(OD_CAL_PATH) or not os.path.exists(TEMP_CAL_PATH) or not os.path.exists(LIGHT_CAL_PATH):
